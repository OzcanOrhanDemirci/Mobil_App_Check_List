    ...
    features: [
      ...
      },
      {
        ...
        howto: {
          ...
          release: {
            tr: "1) GitHub'da branch'i main'e merge etmek için <strong>Pull Request</strong> (PR) aç: branch sayfasında \"Compare & pull request\". 2) PR açıklaması ne yaptığını ve nasıl test edileceğini söyleyen kısa bir özet içersin. 3) <strong>En az 1 takım üyesinden Review</strong> iste; o onayladıktan sonra merge et. 4) GitHub Settings → Rules → <strong>Rulesets</strong> → New branch ruleset ile <code>main</code>'i koru: \"Require a pull request before merging\" + \"Require status checks to pass\" aç. <span class='hint'>Eski \"Branch protection rules\" ekranı hâlâ duruyor ama GitHub artık ruleset'leri öneriyor; mevcut kuralı Settings → Branches'taki \"Convert to ruleset\" ile taşıyabilirsin.</span> 5) Merge sonrası branch'i sil (GitHub'ın \"Delete branch\" butonu); local'de <code>git branch -d feature/&lt;name&gt;</code> ile temizle. 6) Squash merge tercih ediliyorsa Settings → Pull Requests → \"Allow squash merging\"; her özellik tek bir temiz commit olur.",
            en: "1) Open a <strong>Pull Request</strong> (PR) on GitHub to merge the branch into main: click \"Compare & pull request\" on the branch page. 2) The PR description should be a short summary of what you did and how to test it. 3) Request a <strong>Review from at least one teammate</strong>; merge only after their approval. 4) Settings → Rules → <strong>Rulesets</strong> → New branch ruleset to protect <code>main</code>: turn on \"Require a pull request before merging\" + \"Require status checks to pass\". <span class='hint'>The old \"Branch protection rules\" screen still exists, but GitHub now steers you to rulesets; an existing rule can be moved over with \"Convert to ruleset\" under Settings → Branches.</span> 5) After merge, delete the branch (GitHub's \"Delete branch\" button); locally clean it with <code>git branch -d feature/&lt;name&gt;</code>. 6) If you prefer squash merges, Settings → Pull Requests → \"Allow squash merging\"; each feature becomes one clean commit."
          },
          simple: {
            mvp: {
      ...
      },
      {
        ...
        howto: {
          ...
          release: {
            tr: "1) Repo'da bir <code>.env.example</code> dosyası oluştur (gerçek değerler boş): <code>API_KEY=</code>, <code>SUPABASE_URL=</code> gibi anahtar isimleri var ama değerleri YOK. Yeni ekip üyesi bunu kopyalayıp <code>.env</code> olarak adlandırıp kendi değerleriyle dolduruyor. 2) Build / IDE / OS dosyalarını da ignore et: <code>build/</code>, <code>.dart_tool/</code>, <code>node_modules/</code>, <code>.idea/</code>, <code>.vscode/</code> (ortak ayarlar hariç), <code>.DS_Store</code>, <code>Thumbs.db</code>. 3) Geçmişe sızmış anahtarları temizle: <code>git filter-repo</code> (Git artık <code>filter-branch</code>'i önermiyor) veya <code>bfg-repo-cleaner</code> ile geçmişten sil, sonra <code>git push --force</code> (ekibe haber ver). 4) GitHub <strong>Secret scanning</strong> (Settings → Advanced Security → Secret Protection) aktif; public repo'larda ücretsiz ve yeni kişisel repo'larda push protection ile birlikte varsayılan açık. Gelecekteki sızıntılarda otomatik uyarı gelir, sızdıran push engellenir.",
            en: "1) Create a <code>.env.example</code> file in the repo (with empty real values): keys like <code>API_KEY=</code>, <code>SUPABASE_URL=</code> are present without values. A new teammate copies this to <code>.env</code> and fills in their own values. 2) Also ignore build / IDE / OS files: <code>build/</code>, <code>.dart_tool/</code>, <code>node_modules/</code>, <code>.idea/</code>, <code>.vscode/</code> (except shared settings), <code>.DS_Store</code>, <code>Thumbs.db</code>. 3) Clean up keys leaked into history: use <code>git filter-repo</code> (Git itself discourages <code>filter-branch</code>) or <code>bfg-repo-cleaner</code> to remove it from history, then <code>git push --force</code> (warn the team). 4) Enable GitHub <strong>Secret scanning</strong> (Settings → Advanced Security → Secret Protection); it is free on public repositories and on by default, with push protection, for new personal-account public repos. Future leaks get auto-flagged and the offending push is blocked."
          },
          simple: {
            mvp: {
            ...
            release: {
              tr: "1) Repo'na bir <strong>örnek gizli dosya</strong> ekle (<code>.env.example</code>): içeride anahtar isimleri var ama değerleri yok. Yeni ekip üyesi bunu kopyalayıp kendi değerleriyle doldurabilir. 2) Eğer bir anahtar yanlışlıkla GitHub'a sızmış olabilir: o anahtarı <strong>derhal iptal et</strong> ve yenisini üret. 3) GitHub'ın \"Secret scanning\" özelliğini aç (Settings → Advanced Security); herkese açık depolarda ücretsizdir ve ileride bir kaza olursa otomatik uyarı gelir.",
              en: "1) Add a <strong>sample secret file</strong> (<code>.env.example</code>) to your repo: it has the key names but no values. A new teammate copies it and fills in their own values. 2) If a key might have leaked to GitHub: <strong>revoke it immediately</strong> and generate a new one. 3) Enable GitHub's \"Secret scanning\" feature (Settings → Advanced Security); it is free on public repositories and future leaks get auto-flagged."
            }
          }
        }
      },
